...
const updateTimeTable = async (req, res) => {
      ...
      await Promise.all(
        sessionArr.map(async (session) => {
          ...
          if (foundSession.faculty.toString() !== faculty_id.toString()) {
            errors.push(
              `trying to assign a session that does not belongs to this faculty!`
            );
            return;
          }
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ error: "time table id is required! try again with valid id!" });
    }

    //check time table is existing with provided id
    const foundTimeTable = await timeTableModel.findById(id);

    if (!foundTimeTable) {
      return res
        .status(400)
        .json({ error: "time table does not exist belongs to provided id!" });
    }
    ...
    if (updatedTimeTable) {
      const dispatchNotification = await notificationModel.create({
        type: `${foundFaculty.username} ${updatedTimeTable.academic_year} Timetable Updated!`,
        message: `Timetable of ${foundFaculty.username} for academic year ${updatedTimeTable.academic_year} has been updated!. ${updatedTimeTable.academic_year} students can now check their updated time tables.`,
      ...
      return res.status(400).json({ error: "time table can not be updated." });
    }
  } catch (error) {
